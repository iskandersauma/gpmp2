...
def default_base_pose(problemset):
  ...
  if problemset == 'robot':
    return [6.00000000e-03, 6.00000000e-03, 0.0, 1.00000000e-01, -3.53813446e-16]
  else:
    print "Unknown problem set"
    sys.exit()
...
def joint_names(problemset):
  ...
  'l_gripper_r_finger_tip_joint', 'l_gripper_joint', 'torso_lift_motor_screw_joint']
  if problemset == 'robot':
    return ['virtual_joint', 'se_leg_to_ne_leg', 'ne_leg_to_nw_leg', 'nw_leg_to_sw_leg', 'se_leg_to_s_crossbeam', 
  'ne_leg_to_n_crossbeam', 's_crossbeam_to_e_cube', 'e_cube_to_w_cube', 'e_cube_to_s_rail', 'n_crossbeam_to_n_rail', 
  'X', 's_slider_to_e_rail', 'e_rail_to_w_rail', 's_slider_to_n_slider', 'e_rail_to_ne_cube', 'ne_cube_to_se_cube', 
  'ne_cube_to_nw_cube', 'nw_cube_to_sw_cube', 'Y', 'carriage_to_e_carriage_beam', 'e_carriage_beam_to_w_carriage_beam', 
  'e_carriage_beam_to_under_carriage', 'B', 'elbow_to_forearm', 'forearm_to_forearm_motor', 
  'forearm_motor_to_forearm_gearbox', 'Z', 'A', 'yaw_gear_to_ee', 'asp_ee']
  else:
    print "Unknown problem set"
    sys.exit()
...
def states(problemset):
  ...
  if problemset == 'robot':
    starts = 7
    ends = 7
    start = [[0 for i in range(5)] for x in range(starts)]
    end = [[0 for i in range(5)] for x in range(ends)]
    start[0] = [  1.10,   0.01,  1.15, 1.30, -1.15] # start pap1 opposite side of empty container
    start[1] = [1.94, 0.01, -1.18, 1.30, 1.18] #start pap2 close to half full container
    start[2] = [  1.05,   0.01,  -1.40, 1.30, 0.0] #start pap3 three containers
    start[3] = [  1.05,   0.01,  -1.40, 1.30, 0.0]
    start[4] = [  1.10,   0.01,  1.15, 1.30, -1.15] # start pap5 half full container and obstacle
    start[5] = [  1.15,   0.13,  1.0, 1.30, -1.0] # start easy1 only one obstacle to avoid
    start[6] = [  1.90,   0.01,  -1.18, 1.30, 1.18] # start easy2 close to empty container
    end[0]   = [  0.9,  0.01,  -2.18, 1.15, 2.18] #goal pap1
    end[1]   = [0.76, 0.02, -2.13, 1.20, 2.13] #goal pap2
    end[2]   = [  1.7,  0.01,  0.0, 1.25, 1.75] #goal pap3
    end[3]   = [  1.5,  0.01,  0.0, 1.25, 1.75]
    end[4]   = [0.75, 0.20, -2.38, 1.15, 0.81] #goal pap5
    end[5] = [  0.95,   0.13,  -1.0, 1.25, -1.0] #goal easy1 
    end[6]   = [  0.9,  0.01,  -2.18, 1.15, 2.18] #goal easy2 

	#start from rviz
    #start[0] = [  1.10,   0.01,  1.15, 1.30, -1.15] # start pap1 opposite side of empty container
    #start[1] = [1.94, 0.01, -1.18, 1.30, 1.2] #start pap2 close to half full container
    #start[2] = [  1.05,   0.01,  -1.40, 1.30, 0.0] #start pap3 three containers
    #start[3] = [  1.05,   0.01,  -1.40, 1.30, 0.0]
    #start[4] = [  1.10,   0.01,  1.15, 1.30, -1.15] # start pap5 half full container and obstacle
    #start[5] = [  1.15,   0.13,  1.0, 1.30, -1.0] # start easy1 only one obstacle to avoid
    ...
    #end[3]   = [  1.5,  0.01,  0.0, 1.25, 1.75]
    #end[4]   = [0.75, 0.20, -2.38, 1.15, 0.81] #goal pap5
    #end[5] = [  0.95,   0.13,  -1.0, 1.30, -1.0] #goal easy1 
    #end[6]   = [  0.9,  0.01,  -2.18, 1.15, 2.18] #goal easy2
